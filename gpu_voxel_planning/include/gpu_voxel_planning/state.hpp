...
class State {
 public:
  ...
  explicit State(Robot &robot) : robot(robot) {}
  ...
  VictorRightArmConfig getCurConfig() const { return VictorRightArmConfig(current_config); }
  ...
  void updateFreeSpace(const DenseGrid &new_free) { bel->updateFreeSpace(new_free); }
...
class SimulationState : public State {
 public:
  explicit SimulationState(Robot &robot) : State(robot) {}
  ...
  bool move(const VictorRightArmConfig &c, const DenseGrid &true_world, RosInterface &ri) {
    ...
    accumulated_cost += EigenHelpers::Distance(VictorRightArmConfig(current_config).asVector(), c.asVector());
    robot.set(c.asMap());
 ...
 public:
  ...
  bool move(const std::vector<VictorRightArmConfig> &path, RosInterface &ri) {
    ...
    if (resp.ci.collided) {
      ...
      accumulated_cost += EigenHelpers::Distance(path.front().asVector(), end_config.asVector());
    ...
    accumulated_cost += EigenHelpers::Distance(path.front().asVector(), path.back().asVector());
