...
class GpuVoxelsVictor {
 public:
  ...
  void addLinks(const VictorConfig &config, const std::vector<std::string> &link_names, const std::string &map_name);

  void addCHS(const PathUtils::Path &path, const std::vector<std::string> &collision_links);

  void addCHSToMap(const PathUtils::Path &path, const std::vector<std::string> &collision_links,
