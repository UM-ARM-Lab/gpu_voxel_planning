...
class Box {
 public:
  ...
  double z;
  double sl;
  Box(double x_, double y_, double z_, double side_length_) : x(x_), y(y_), z(z_), sl(side_length_){};

  Vector3f lower() const { return Vector3f(x, y, z); };
  Vector3f upper() const { return Vector3f(x + sl, y + sl, z + sl); };
};
...
class BoxValidator : public ompl::base::StateValidityChecker, public ompl::base::MotionValidator {
 public:
  BoxValidator(const ompl::base::SpaceInformationPtr &si, BoxWorld *box_world);
...
class BoxPathValidator : public ompl::geometric::PathValidator {
 public:
  BoxPathValidator(ompl::base::SpaceInformationPtr si, BoxWorld *box_world);
  virtual bool checkPath(const std::vector<ompl::base::State *> path, size_t &collision_index) override;

  virtual double getPathCost(const std::vector<ompl::base::State *> path, size_t &collision_index) override;
...
class BoxMinVoxObjective : public ompl::base::OptimizationObjective {
 public:
  BoxMinVoxObjective(ompl::base::SpaceInformationPtr si, BoxWorld *box_world);
...
class BoxMinColProbObjective : public ompl::base::OptimizationObjective {
 public:
  BoxMinColProbObjective(ompl::base::SpaceInformationPtr si, BoxWorld *box_world);
  virtual ompl::base::Cost motionCost(const ompl::base::State *s1, const ompl::base::State *s2) const;
  virtual ompl::base::Cost stateCost(const ompl::base::State *state) const;
...
class BoxMinColProbSweptObjective : public ompl::base::OptimizationObjective {
 public:
  BoxMinColProbSweptObjective(ompl::base::SpaceInformationPtr si, BoxWorld *box_world);
  virtual ompl::base::Cost motionCost(const ompl::base::State *s1, const ompl::base::State *s2) const;
  virtual ompl::base::Cost stateCost(const ompl::base::State *state) const;
...
class BoxPlanner {
 public:
  ...
  virtual Maybe::Maybe<ompl::base::PathPtr> planPath(ompl::base::ScopedState<> start, ompl::base::ScopedState<> goal);

  Maybe::Maybe<ompl::base::PathPtr> planPathDouble(std::vector<double> start, std::vector<double> goal);
...
template <class T>
BoxTRRT<T>::BoxTRRT(BoxWorld *box_world) : BoxPlanner(box_world) {
  initializePlanner();
