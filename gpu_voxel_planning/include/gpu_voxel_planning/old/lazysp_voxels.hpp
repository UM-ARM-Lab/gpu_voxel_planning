...
bool isValidFromKnownObs(std::vector<double> start, std::vector<double> end, GpuVoxelsVictor *victor) {
...
bool checkEdge(arc_dijkstras::GraphEdge &e, arc_dijkstras::Graph<std::vector<double>> &g, GpuVoxelsVictor *victor) {
  bool valid = isValidFromKnownObs(g.GetNodeImmutable(e.GetFromIndex()).GetValueImmutable(),
                                   g.GetNodeImmutable(e.GetToIndex()).GetValueImmutable(), victor);
  e.SetValidity(valid ? arc_dijkstras::EDGE_VALIDITY::VALID : arc_dijkstras::EDGE_VALIDITY::INVALID);
  return valid;
}

double calculateEdgeWeight(arc_dijkstras::Graph<std::vector<double>> &g, arc_dijkstras::GraphEdge &e,
...
double evaluateEdge(arc_dijkstras::Graph<std::vector<double>> &g, arc_dijkstras::GraphEdge &e,
                    GpuVoxelsVictor *victor) {
...
std::vector<int64_t> planPath(int start, int goal, HaltonGraph &g, GpuVoxelsVictor *victor) {
  ...
  const auto eval_fn = [&victor](arc_dijkstras::Graph<std::vector<double>> &g, arc_dijkstras::GraphEdge &e) {
    return evaluateEdge(g, e, victor);
  };

  auto result =
      arc_dijkstras::LazySP<std::vector<double>>::PerformLazySP(g, start, goal, &distanceHeuristic, eval_fn, true);
  return result.first;
