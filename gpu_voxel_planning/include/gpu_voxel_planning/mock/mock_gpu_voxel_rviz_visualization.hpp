...
class MockGpuVoxelRvizVisualizer : public GpuVoxelRvizVisualizer {
 public:
  ...
  void vizEEPath(const std::vector<GVP::VictorRightArmConfig> path_config, std::string path_name) override {}

  void vizGrid(const DenseGrid& grid, const std::string& name, const std_msgs::ColorRGBA& color) const override {}

  void vizChs(const std::vector<DenseGrid>& chss, const std::string& ns = "chs") const override {}
};
}  // namespace GVP
