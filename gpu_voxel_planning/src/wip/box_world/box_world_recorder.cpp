...
bool testPlanner(std::string logger) {
  ...
  boxWorld->gvl.reset();
  if (success) PROFILE_RECORD(logger);
  std::cout << "Finished " << logger << "\n";
  return success;
}

bool testLBKPIECE() { return testPlanner<BoxLBKPIECE>(LBKPIECE_TIME); }

bool testLRRTF() { return testPlanner<BoxLazyRRTF>(LazyRRTF_TIME); }

bool testMinVoxRRTstar() { return testPlanner<BoxRRTstar<BoxMinVoxObjective>>(RRTSTAR_MINVOX_TIME); }

bool testMinColProbRRTstar() { return testPlanner<BoxRRTstar<BoxMinColProbObjective>>(RRTSTAR_MINPROB_TIME); }

bool testMinColProbSweptRRTstar() {
  return testPlanner<BoxRRTstar<BoxMinColProbSweptObjective>>(RRTSTAR_MINPROB_SWEPT_TIME);
}

bool testMinColProbSweptTRRT() { return testPlanner<BoxTRRT<BoxMinColProbSweptObjective>>(TRRT_MINPROB_SWEPT_TIME); }
