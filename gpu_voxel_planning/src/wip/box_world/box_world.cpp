...
BoxWorld::BoxWorld() : num_observed_sets(0) {
...
bool BoxWorld::isActualCollision() {
  size_t num_colls = gvl->getMap(BOX_ACTUAL_MAP)
                         ->as<voxelmap::ProbVoxelMap>()
                         ->collideWith(gvl->getMap(OBSTACLES_ACTUAL_MAP)->as<voxelmap::ProbVoxelMap>());
  return num_colls > 0;
}
...
void BoxWorld::resetQuery() { gvl->clearMap(BOX_QUERY_MAP); }
...
std::vector<size_t> BoxWorld::countSeenCollisionsInQueryForEach() {
  ...
  for (int i = 0; i < num_observed_sets; i++) {
    query_collisions[i] = gvl->getMap(BOX_QUERY_MAP)
                              ->as<voxelmap::ProbVoxelMap>()
                              ->collideWith(gvl->getMap(SEEN_OBSTACLE_SETS[i])->as<voxelmap::ProbVoxelMap>());
  }
  return query_collisions;
}

size_t BoxWorld::countVoxels(const std::string &map_name) { return countIntersect(FULL_MAP, map_name); }

size_t BoxWorld::countIntersect(const std::string &map_1, const std::string &map_2) {
  return gvl->getMap(map_1)->as<voxelmap::ProbVoxelMap>()->collideWith(
      gvl->getMap(map_2)->as<voxelmap::ProbVoxelMap>());
}
 ...
 **               VALIDATOR                   **
 ***********************************************/
BoxValidator::BoxValidator(const ob::SpaceInformationPtr &si, BoxWorld *box_world)
    : ob::StateValidityChecker(si), ob::MotionValidator(si) {
  spi_ptr = si;
...
bool BoxValidator::checkMotion(const ompl::base::State *s1, const ompl::base::State *s2,
  ...
  for (int j = 1; j <= nd; j++) {
    ...
    if (!isValid(test)) {
      ...
      if (lastValid.first != nullptr) stateSpace->interpolate(s1, s2, lastValid.second, lastValid.first);
      result = false;
      break;
    }
  }
 ...
 **            PATH VALIDATOR                 **
 ***********************************************/
BoxPathValidator::BoxPathValidator(ob::SpaceInformationPtr si, BoxWorld *box_world) : PathValidator(si) {
  box_world_ptr = box_world;
}

void BoxPathValidator::setProbabilityThreshold(double th) { threshold = th; }

double BoxPathValidator::getPathCost(const std::vector<ompl::base::State *> path, size_t &collision_index) {
  std::cout << "get path cost not implemented for box world\n";
  assert(false);
}

bool BoxPathValidator::checkPath(const std::vector<ompl::base::State *> path, size_t &collision_index) {
...
BoxMinVoxObjective::BoxMinVoxObjective(ompl::base::SpaceInformationPtr si, BoxWorld *box_world)
...
ob::Cost BoxMinVoxObjective::motionCost(const ob::State *s1, const ob::State *s2) const {
...
BoxMinColProbObjective::BoxMinColProbObjective(ompl::base::SpaceInformationPtr si, BoxWorld *box_world)
    : ob::OptimizationObjective(si) {
  spi_ptr = si;
  box_world_ptr = box_world;
}
...
ob::Cost BoxMinColProbObjective::motionCost(const ob::State *s1, const ob::State *s2) const {
  ompl::base::StateSpace *stateSpace_ = spi_ptr->getStateSpace().get();
  assert(stateSpace_ != nullptr);
...
BoxMinColProbSweptObjective::BoxMinColProbSweptObjective(ompl::base::SpaceInformationPtr si, BoxWorld *box_world)
    : ob::OptimizationObjective(si) {
  spi_ptr = si;
  box_world_ptr = box_world;
...
ob::Cost BoxMinColProbSweptObjective::motionCost(const ob::State *s1, const ob::State *s2) const {
...
Maybe::Maybe<ob::PathPtr> BoxPlanner::planPathDouble(std::vector<double> start, std::vector<double> goal) {
...
Maybe::Maybe<ob::PathPtr> BoxPlanner::planPath(ompl::base::ScopedState<> start, ompl::base::ScopedState<> goal) {
...
BoxLBKPIECE::BoxLBKPIECE(BoxWorld *box_world) : BoxPlanner(box_world) { initializePlanner(); }
...
void BoxLBKPIECE::postPlan(ob::PathPtr path) { simp_->simplifyMax(*(path->as<og::PathGeometric>())); }

/***********************************************
 **               Box LazyRRTF                **
 ***********************************************/

BoxLazyRRTF::BoxLazyRRTF(BoxWorld *box_world) : BoxPlanner(box_world) { initializePlanner(); }
...
Maybe::Maybe<ob::PathPtr> BoxLazyRRTF::planPath(ompl::base::ScopedState<> start, ompl::base::ScopedState<> goal) {
  preparePlanner(start, goal);
  ob::PathPtr path;
  int planning_time = 5;
...
template <class T>
BoxRRTstar<T>::BoxRRTstar(BoxWorld *box_world) : BoxPlanner(box_world) {
  initializePlanner();
...
int main(int argc, char *argv[]) {
  ...
  if (reached_goal) std::cout << "REACHED GOAL!\n";
