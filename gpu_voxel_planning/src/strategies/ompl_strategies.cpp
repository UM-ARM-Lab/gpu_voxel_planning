...
Path OMPL_Strategy::applyTo(Scenario &scenario, GpuVoxelRvizVisualizer &viz_) {
  ...
  ob::ScopedState<> start = ompl_utils::toScopedState(scenario.getState().getCurConfig().asVector(), space);
  ob::ScopedState<> goal = ompl_utils::toScopedState(VictorRightArmConfig(scenario.goal_config).asVector(), space);
  ...
  PROFILE_RECORD_DOUBLE("SetRobotConfig before smoothing", arc_utilities::Profiler::getData("Set robot config").size());

  PathUtils::Path pu_path = ompl_utils::omplPathToDoublePath(&ss.getSolutionPath(), ss.getSpaceInformation());
...
bool OMPL_Strategy::isOmplStateValid(const ompl::base::State *ompl_state, GVP::State &gvp_state) {
...
std::string RRT_Strategy::getName() const { return "RRT_Strategy"; }
...
std::string BIT_Strategy::getName() const { return "BIT_Strategy"; }

Path BIT_Strategy::smooth(Path gvp_path, State &state) { return GVP::densify(gvp_path, discretization); }
...
std::string SPARS_Strategy::getName() const { return "SPARS_Strategy"; }

Path SPARS_Strategy::smooth(Path gvp_path, State &state) { return GVP::densify(gvp_path, discretization); }
...
std::string SST_Strategy::getName() const { return "SST_Strategy"; }
...
std::string LBKPIECE_Strategy::getName() const { return "LBKPIECE_Strategy"; }
...
std::string PRM_Strategy::getName() const { return "PRM_Strategy"; }
...
std::string STRIDE_Strategy::getName() const { return "STRIDE_Strategy"; }
