...
void GraphSearchStrategy::updateGoals(const Scenario &scenario) {
  ...
  if (goal_configs.empty()) {
    std::cout << arc_color::RED << "No goal configurations\n" << arc_color::RESET;
...
Path OmniscientGraphSearch::applyTo(Scenario &scenario, GpuVoxelRvizVisualizer &viz) {
  ...
  for (size_t i = 0; i < node_path.size() - 1; i++) {
    Path segment = interpolate(VictorRightArmConfig(graph.getNode(node_path[i]).getValue()),
...
std::vector<NodeIndex> ThompsonGraphSearch::plan(NodeIndex start, std::vector<NodeIndex> goals, State &s,
  ...
  for (int i = 0; i < 100; i++) {
    ...
    viz.vizEEPath(interpolate(VictorRightArmConfig(s.getCurConfig()),
                              VictorRightArmConfig(graph.getNode(result[1]).getValue()), 0.1),
