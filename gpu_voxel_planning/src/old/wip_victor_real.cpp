...
bool attemptGoal(VictorPlanner& planner, std::vector<double> goal) {
  ...
  while (!reached_goal) {
    if (DO_CONTROL) {
      ...
      Optpath maybe_path = planner.localControlConfig(real_world->victor_model.cur_config, goal_config);
      while (maybe_path.Valid()) {
        ...
        maybe_path = planner.localControlConfig(real_world->victor_model.cur_config, goal_config);
      }
    ...
    if (DO_PLAN) {
      ...
      Optpath maybe_path = planner.planPathConfig(real_world->victor_model.cur_config, goal_config);
