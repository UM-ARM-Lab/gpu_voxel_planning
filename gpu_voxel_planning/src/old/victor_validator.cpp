...
VictorValidator::VictorValidator(const ob::SpaceInformationPtr &si, GpuVoxelsVictor *victor_model)
    : ob::StateValidityChecker(si), ob::MotionValidator(si), si_(si) {
  // si_ = si;
...
VictorValidator::~VictorValidator() { std::cout << "Deleting Validator\n"; }
 ...
 ***********************************************/
VictorPathProbCol::VictorPathProbCol(const ob::SpaceInformationPtr &si, GpuVoxelsVictor *victor_model)
    : PathValidator(si) {
  victor_model_ = victor_model;
}

double VictorPathProbCol::getPathCost(const std::vector<ob::State *> path, size_t &collision_index, bool fast) {
 ...
 */
double VictorPathProbCol::getPathCost(const std::vector<ob::State *> path, std::vector<double> &costs, bool fast) {
...
bool VictorPathProbCol::checkPath(const std::vector<ob::State *> path, size_t &collision_index) {
 ...
 **        PATH VOXEL COUNT                   **
 ***********************************************/
VictorPathVox::VictorPathVox(const ob::SpaceInformationPtr &si, GpuVoxelsVictor *victor_model) : PathValidator(si) {
  victor_model_ = victor_model;
}

double VictorPathVox::getPathCost(const std::vector<ob::State *> path, size_t &collision_index, bool fast) {
  std::vector<double> costs;
  double c = getPathCost(path, costs, fast);
  collision_index = costs.size();
  return c;
 ...
 *  Early termination if prob of collision is above threshold
 */
double VictorPathVox::getPathCost(const std::vector<ob::State *> path, std::vector<double> &costs, bool fast) {
...
bool VictorPathVox::checkPath(const std::vector<ob::State *> path, size_t &collision_index) {
  return getPathCost(path, collision_index) <= threshold;
}
...
ob::Cost VictorObjective::motionCost(const ob::State *s1, const ob::State *s2) const {
  size_t col_index;
  bool fast = true;
...
ob::Cost VictorObjective::stateCost(const ob::State *state) const { return motionCost(state, state); }
