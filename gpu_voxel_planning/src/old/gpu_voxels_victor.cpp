...
std::vector<std::string> i_right_arm_joint_names{
...
std::vector<std::string> i_right_arm_collision_link_names{"victor_right_arm_link_3",
...
std::vector<std::string> i_right_gripper_collision_link_names{// "victor_right_arm_link_6",
                                                              "victor_right_arm_link_7",
                                                              "victor_right_gripper_palm",
                                                              "victor_right_gripper_mounting_bracket",
                                                              "victor_right_gripper_fingerA_base",
...
std::vector<std::string> i_left_arm_joint_names{
...
GpuVoxelsVictor::GpuVoxelsVictor()
  ...
  if (PEG_IN_HOLE || REAL_ROBOT) {
    gvl->addRobot(VICTOR_ROBOT, ros::package::getPath("gpu_voxel_planning") + "/urdf/victor_right_arm_with_rod.urdf",
                  false);
    right_gripper_collision_link_names.push_back("rod");
    right_arm_collision_link_names.push_back("rod");
  } else {
    gvl->addRobot(VICTOR_ROBOT, ros::package::getPath("gpu_voxel_planning") + "/urdf/victor_right_arm_only.urdf",
                  false);
  }

  // gvl->addRobot(VICTOR_ROBOT, ros::package::getPath("gpu_voxel_planning") + "/urdf/victor.urdf", false);

  gvl->addRobot(VICTOR_ROBOT_STATIONARY,
                ros::package::getPath("gpu_voxel_planning") + "/urdf/victor_left_arm_and_body.urdf", false);
...
void GpuVoxelsVictor::resetQuery() { gvl->clearMap(VICTOR_QUERY_MAP); }

void GpuVoxelsVictor::addQueryState(const VictorConfig &c) { insertVictorIntoMap(c, VICTOR_QUERY_MAP); }
...
size_t GpuVoxelsVictor::countVoxels(const std::string &map_name) { return countIntersect(FULL_MAP, map_name); }

bool GpuVoxelsVictor::queryFreeConfiguration(const VictorConfig &c) { return countTotalCHSCollisionsForConfig(c) == 0; }
...
void GpuVoxelsVictor::addQueryLink(const VictorConfig &c, const std::string &link_name) {
  gvl->setRobotConfiguration(VICTOR_ROBOT, c);
...
void GpuVoxelsVictor::addLinks(const VictorConfig &config, const std::vector<std::string> &link_names,
...
void GpuVoxelsVictor::addCHS(const Path &path, const std::vector<std::string> &collision_links) {
...
void GpuVoxelsVictor::addCHSToMap(const Path &path, const std::vector<std::string> &collision_links,
...
// {
//     boost::shared_ptr<voxelmap::ProbVoxelMap> victor =
//     boost::dynamic_pointer_cast<voxelmap::ProbVoxelMap>(gvl->getMap(VICTOR_ACTUAL_MAP));

//     boost::shared_ptr<voxelmap::DistanceVoxelMap> dist_map =
//     boost::dynamic_pointer_cast<voxelmap::DistanceVoxelMap>(gvl->getMap(OBSTACLE_DISTANCE_MAP));
...
void GpuVoxelsVictor::doVis() {
  ...
  for (int i = 0; i < num_observed_chs; i++) gvl->visualizeMap(COLLISION_HYPOTHESIS_SETS[i]);
}
...
void GpuVoxelsVictor::visPath(const Path &path) {
  ...
  if (path.size() == 0) {
    gvl->insertBoxIntoMap(Vector3f(0.0, 0.0, 0.), Vector3f(0.1, 0.1, 0.1), VICTOR_PATH_SOLUTION_MAP,
                          eBVM_SWEPT_VOLUME_START, 2);
    usleep(100000);
  ...
  for (size_t step = 0; step < path.size(); step++) {
    ...
    gvl->insertRobotIntoMap(VICTOR_ROBOT, VICTOR_PATH_SOLUTION_MAP,
...
void GpuVoxelsVictor::hidePath() { visPath(Path()); }
