...
//  grid_pub = n.advertise<rviz_voxelgrid_visuals_msgs::VoxelgridStamped>("grid", 10);
...
void GVP::GpuVoxelRvizVisualizer::vizGrid(const DenseGrid& grid, const std::string& name,
  ...
  RVV::voxelgridToFloatArray();
  grid_pub.publish(visualizeDenseGrid(grid, global_frame, name, color));
}

void GVP::GpuVoxelRvizVisualizer::vizGrid(const std::vector<Vector3f>& centers, const float side_length,
                                          const std::string& name, const std_msgs::ColorRGBA& color) const {
